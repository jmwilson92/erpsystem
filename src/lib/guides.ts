  ...
  // ──────────────────────────── Supply Chain ───────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "The PR routes to whoever owns the charge — a WBS owner, program, or budget holder — never to purchasing itself. The Purchasing workbench shows each PR's approval status.",
  ...
  // ─────────────────────────── Quality & Compliance ────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        why: "You can also pin a suspect calibration tool to the case, or flag it as ESD/FOD/counterfeit-caused to auto-open that program's incident.",
      },
      {
        route: "/mrb",
        selector: HEADER,
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Track gages and instruments with intervals and auto-rolling due dates. Attach the calibration certificate right on the record, and print a label or download a DXF to laser-etch the ID.",
        why: "A passing calibration rolls the next-due date forward automatically, so the register is always current.",
      },
      {
        route: "/quality/programs/tools",
        selector: HEADER,
        title: "Tool control & toolboxes",
        body: "Group tools into toolboxes, run a tool check that ticks each tool and saves the report to history, and toggle 'needs calibration' to pull a tool onto the calibration register.",
      },
      {
        route: "/quality/programs/tools",
        selector: HEADER,
        title: "Missing / broken tool reports",
        body: "Report a tool broken (gather its pieces, one-click a replacement PR) or missing (auto-opens a FOD incident). An unrecoverable piece also escalates to FOD.",
    ...
    steps: [
      {
        ...
        selector: HEADER,
        title: "ESD stations & inspections",
        body: "Define an inspection template of check points once; it applies to every station. Click 'Inspect' on a station to run it with pass/fail, notes, and photos — saved to a clickable history.",
      },
      {
        route: "/quality/programs/esd",
        selector: HEADER,
        title: "Humidity tracking",
        body: "A humidity device can POST relative humidity per area to the ESD module (or you log it by hand). Readings show against the 30–70% ESD-safe band, flagging out-of-band areas.",
      },
      {
        route: "/quality/programs/fod",
        selector: HEADER,
        title: "FOD walks",
        body: "FOD zones use the same template + walk workflow — customize what to look for, attach photos, and save each walk to history.",
        why: "ESD- or FOD-caused MRB failures auto-open an incident here that runs the disposition steps you define.",
      },
      {
        route: "/quality/programs/audits",
        selector: HEADER,
        title: "Internal audits",
        body: "Run a step-by-step audit against a clause; mark each OK, OFI, or NCR with evidence photos. Every NCR opens a tracked corrective action with a reinspect-by date; OFIs are logged.",
      },
      {
      ...
      {
        route: "/quality/programs/safety",
        selector: HEADER,
        title: "Policies, everywhere",
        body: "Every program carries a policy document controlled in Config Management — it revises through the same ECR process as work instructions, and stays clickable right on the module.",
      },
    ],
  },
