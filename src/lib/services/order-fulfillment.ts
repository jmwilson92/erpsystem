...
export async function completeWorkOrderToStock(params: {
  ...
  // Serialized parts: every finished unit leaves putaway with a serial.
  // Units serialized during the build keep theirs; any unit still blank
  // gets one auto-minted here so nothing unserialized reaches stock.
  let unitSerials: { serial: string }[] = [];
  if (wo.part?.isSerialized && wo.partId) {
    const { ensureWorkOrderUnits, assignUnitSerial } = await import(
      "./serials"
    );
    ...
    for (const unit of units) {
      if (!unit.serialId) {
        await assignUnitSerial({
          ...
          serial: `SN-${wo.number}-${String(unit.unitIndex).padStart(2, "0")}`,
          userId: params.userId,
        });
      }
    }
    const done = await prisma.workOrderUnit.findMany({
      where: { workOrderId: wo.id },
      include: { serial: { select: { serial: true } } },
      orderBy: { unitIndex: "asc" },
    });
    ...
    await prisma.serialNumber.updateMany({
      where: { id: { in: done.map((u) => u.serialId).filter((x): x is string => !!x) } },
      data: { status: "IN_STOCK", lotNumber },
    });
    unitSerials = done
      .map((u) => (u.serial ? { serial: u.serial.serial } : null))
