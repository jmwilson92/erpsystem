      ...
      {!setupDone && (
        ...
        >
          <span className="text-sm text-slate-200">
            Make it yours — run the 5-step setup wizard: company name, pay periods
            and overtime, review cycles, and your org chart. Takes about 3 minutes.
          </span>
          ...
          {businessPriorities.map((p, i) => (
            <span key={p.id} className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-300">
                <span className="font-mono text-slate-600">{p.number}</span> {p.title}
              </span>
            </span>
          ))}
        </Link>
      ...
      <div data-dash="ops-stats" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Link href="/inventory">
          <StatCard
            ...
            value={lowStock}
            subtitle="Locations at or under 5 available"
            icon={Package}
            accent="amber"
          />
        </Link>
