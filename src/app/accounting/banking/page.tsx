...
export default async function BankingPage({
  ...
  if (!user) return null;
  const canView = await userHasPermission(user.id, "accounting.view");
  const canPost = await userHasPermission(user.id, "accounting.journal.post");
  if (!canView) notFound();

  const sp = searchParams ? await searchParams : {};
  const selectedId = (Array.isArray(sp.acct) ? sp.acct[0] : sp.acct) || "";

  const [overview, accounts, categoryAccounts] = await Promise.all([
    getBankingOverview(),
    ...
    prisma.account.findMany({
      where: { isActive: true, type: { in: ["EXPENSE", "REVENUE", "COGS", "ASSET", "LIABILITY"] } },
      orderBy: { code: "asc" },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Banking"
        description="Connected bank and credit-card feeds. Import transactions, categorize them into the GL, and reconcile."
        actions={
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {overview.map((a) => (
          <Link key={a.id} href={`/accounting/banking?acct=${a.id}`}>
            <Card
              className={
                a.id === active
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="pb-2">
            ...
            <p className="text-xs text-slate-500">
              Categorize a transaction to post it to the GL (balances against the
              account&apos;s cash/CC-payable account), then reconcile.
            </p>
          </CardHeader>
          <CardContent>
            {feed.length === 0 ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {t.status === "UNMATCHED" && canPost ? (
                        ...
                        >
                          ...
                          <select name="categoryAccountId" required className={selectClass} defaultValue="">
            ...
            <Card>
              ...
              <CardContent>
                <form action={actionConnectBank} className="grid gap-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <select name="kind" className="h-9 rounded-lg border border-slate-700 bg-slate-950 px-2 text-sm text-slate-200" defaultValue="CHECKING">
                      <option value="CHECKING">Checking</option>
                      <option value="SAVINGS">Savings</option>
                      <option value="CREDIT_CARD">Credit card</option>
                    </select>
                    <select name="glAccountId" className="h-9 rounded-lg border border-slate-700 bg-slate-950 px-2 text-sm text-slate-200" defaultValue="">
